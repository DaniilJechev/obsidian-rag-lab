...
| MLOPS-002 | P1 | in-progress | 10 | Human scores + RAGAS runtime + bake-off 2–3 OpenRouter generate-моделей (Sprint 23) | Human 10 id заполнены (`sprint22_sample.yaml`). JSON-судья 0–5 остаётся. Добавить `judge_backend: ragas` (Faithfulness/Answer Relevancy пакетом, шкала 0–1; Context P/R — note-level proxy). Затем таблица 2–3 моделей на том же gold/k/ragas-judge; контроль mini — новый ragas-прогон, не JSON Sprint 22. Issue [#59](https://github.com/DaniilJechev/obsidian-rag-lab/issues/59) |
